"""Run OFAC Non-SDN ingestion as a standalone script."""
...
async def main() -> int:
    logger.info("starting_ofac_nonsdn_ingestion", data_dir=str(config.data_dir))

    async with async_session_factory() as session:
        result = await ingest_ofac_nonsdn(session=session, data_dir=config.data_dir)
